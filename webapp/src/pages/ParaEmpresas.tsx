...
const ParaEmpresas: React.FC = () => {
    ...
    return (
        <div className="pe-page">
            <style>{`
                .pe-page { background: var(--bg-primary); color: var(--text-primary); min-height: 100vh; position: relative; overflow-x: hidden; font-family: 'Outfit', sans-serif; }
                ...
                .ent-card { padding: 40px; border-radius: 32px; background: rgba(255,255,255,0.55); backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.7); transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1); box-shadow: 0 4px 16px rgba(0,40,120,0.04); transform-style: preserve-3d; }
                .ent-card:hover { transform: translateY(-10px) rotateX(2deg); background: rgba(255,255,255,0.8); border-color: rgba(0, 85, 255, 0.2); box-shadow: 0 24px 48px rgba(0,40,120,0.1); }
            ...
            <section className="hero-section">
                <div className="hero-glass reveal">
                    ...
                    <p style={{ color: 'var(--text-secondary)', fontSize: '1.2rem', maxWidth: '700px', margin: '0 auto 40px' }}>Tecnologia de ponta e uma malha robusta para que você foque nas vendas enquanto cuidamos de toda a sua entrega.</p>
                    <Link to="/api-ecommerce" className="btn-primary" style={{ padding: '18px 48px', fontSize: '1.1rem', whiteSpace: 'nowrap', display: 'inline-block' }}>Consultar API</Link>
                </div>
            </section>
            ...
            <div className="enterprise-grid">
                {[
                    ...
                    <div key={i} className={`ent-card reveal reveal-delay-${i + 1}`}>
                        ...
                        <h3>{s.title}</h3>
                        <p>{s.desc}</p>
                        <Link to="/api-ecommerce" style={{ color: '#0055ff', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none' }}>Ver detalhes <ArrowRight size={16} /></Link>
                    </div>
                ))}
            </div>
